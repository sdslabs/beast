...
func DeployChallenge(challengeName string) error {
	...
	if challenge.ContainerId != utils.GetInvalidContainerID(challengeName) && challenge.ContainerId != "" {
		...
		if len(containers) == 1 {
			...
			if err = database.Db.Model(&challenge).Update("ContainerId", utils.GetInvalidContainerID(challengeName)).Error; err != nil {
	...
	if challenge.ImageId != utils.GetInvalidImageID(challengeName) && challenge.ImageId != "" {
		...
		if imageExist {
			...
			if err = database.Db.Model(&challenge).Update("ImageId", utils.GetInvalidImageID(challengeName)).Error; err != nil {
				log.Errorf("Error while saving challenge state in database : %s", err)
				return errors.New("DATABASE ERROR")
			}
		}
...
func undeployChallenge(challengeName string, purge bool) error {
	...
	// So this....
	if challenge.ContainerId == utils.GetInvalidContainerID(challengeName) {
	...
	tx := database.Db.Model(&challenge).Updates(map[string]interface{}{
		...
		"ContainerId": utils.GetInvalidContainerID(challengeName),
	})
