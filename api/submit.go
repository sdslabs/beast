...
func submitFlagHandler(c *gin.Context) {
	...
	if state == 1 {
		...
		if challenge.DynamicFlag {
			...
			// Case 1: Flag exists in submissions but not in validFlags (cheating with invalid flag)
			if !flagInValidFlags && flagInSubmissions {
				if user.ID != submissions[0].UserID {
					...
					msg := "User " + user.Username + " has submitted the flag " + flag + " for challenge " + challenge.Name + " which was previously submitted by user " + subuser.Username
			...
			// Case 2: Flag not in validFlags and not in submissions , just incorrect.
			if !flagInValidFlags && !flagInSubmissions {
				UserChallengesEntry := database.UserChallenges{
					CreatedAt:   time.Now(),
					UserID:      user.ID,
			...
			// Case 3: Flag in validFlags and in submissions, cheating with valid flag
			if flagInValidFlags && flagInSubmissions {
				if user.ID != submissions[0].UserID {
					subuser, _ := database.QueryUserById(submissions[0].UserID)
			...
			// Case 4: Flag in validFlags but not in submissions (solved without cheating) and saved at the end.
